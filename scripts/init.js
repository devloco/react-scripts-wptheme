...
const wptheme = require("@devloco/react-scripts-wptheme/scripts/init.wptheme");
...
module.exports = function(appPath, appName, verbose, originalDirectory, template) {
    ...
    fs.move(path.join(appPath, "gitignore"), path.join(appPath, ".gitignore"), [], (err) => {
        ...
        // wptheme; need to hook into the init process here because fs.move is async
        wptheme(appPath, appName, verbose, originalDirectory, template, readmeExists);
    });
    ...
    // wptheme; end this script early. The wptheme function called from fs.move above will show the final message.
    return;
